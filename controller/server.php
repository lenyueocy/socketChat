<?php
class server{
    //const ERROR_PATH = '/Users/lenyue/E HD/www/chats/log/';
    ...
    public function index(){
        global $config;
        $port = isset($config['chat']['port']) ? $config['chat']['port'] : 9090;
        setcookie('port',$port);
        //model::table('chat')->ceshi()->ceshi1()->ceshi2()->ceshi3()->ceshi4();
    ...
    public function socket() {
        ...
        try {
            ...
            // 将IP和端口绑定在服务器socket上;
            socket_bind($this->master, $chat_host, $port);
    ...
    public function handShake($socket, $buffer) {
        ...
        //socket_write($socket,$this->build(json_encode(['type' => 'handshake','content' => 'done'])),strlen($this->build(json_encode(['type' => 'handshake','content' => 'done']))));
    ...
    /**
     * 拼装信息  有登录login 有退出loginout 有user
     *
     */
    private function dealMsg($socket, $recv_msg) {
        ...
        switch ($msg_type) {
            case 'login':
                ...
                $response['time'] = time();
                $response['ip'] = $this->sockets[(int)$socket]['ip'];
                break;
            case 'logout':
                $user_list = array_column($this->sockets, 'uname');
     ...
     * @param $data
     */
    public function addMsg($data = null){
        switch ($data['type']){
            case 'login':
                $sql_c = "select * from user where name='{$data['content']}'";
                /*if(model::table('user')->query($sql_c)){
                    return false;
                }*/
                $sql_us = "insert into user(id,name,ip,createtime) VALUE (null,'{$data['content']}','{$data['ip']}','{$data['time']}')";
                $re=model::table('user')->save();
//                $user_id = mysqli_insert_id();

                break;
        }
//        $sql="INSERT INTO chat(id,user_id,content,time,type) value(null,{$data['user_id']},'{$data['content']}',{$data['time']},'{$data['type']}');";
        /*$result = model::table('chat')->query($sql);
        return $result;*/
    }
    /**
     * 记录debug信息
    ...
    private function debug(array $info) {
        ...
        file_put_contents(config::get('chat')['logpath'] . 'websocket_debug.log', implode(' | ', $info) . "\r\n", FILE_APPEND);
    }
    ...
    private function error(array $info) {
        ...
        $info = array_map('json_encode', $info);
        file_put_contents(config::get('chat')['logpath']. 'websocket_error.log', implode(' | ', $info) . "\r\n", FILE_APPEND);
    }
    public function errlog($content){
        if(!is_array($content)){
